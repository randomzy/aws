...
let rootChords = ['A','A#','Bb','B','C','C#','Db','D','D#','Eb','E','F','F#','Gb','G','G#','Ab'];
let typeChord = ['','m','7','maj7','m7','sus4','5','dim','sus2','7sus4','9','7#9','m9','maj9','maj6','m6'];
let rootSelect = document.querySelector(".root-select");
let bassSelect = document.querySelector(".bass-select");
let typeSelect = document.querySelector(".type-select");

rootChords.forEach(element => {
...
class ChordHandler{
    ...
    deleteSelected() {
      let selectedId = Array.from(this.selectedDiv.parentNode.children).indexOf(this.selectedDiv);
      console.log(selectedId);
      this.data.remove(selectedId);
      let previousDiv = this.selectedDiv.previousSibling;
      console.log(previousDiv);
      var numberPattern = /\d+/g;
      let curWidth = this.selectedDiv.style.minWidth.match(numberPattern);
      let prevWidth = previousDiv.style.minWidth.match(numberPattern);
      let newWidth = parseInt(curWidth) + parseInt(prevWidth);
    ...
    insertAtCurrentLocation() {
      //previous needs to become shorter
    }
    amendSelected() { 
      //previous needs to change lenght
    ...
    makeDivs(){
        let chordsContainer = document.querySelector(".reels-chords");
        let prevTime = 0;
        let id = 0;
        this.data.forEach(element => {
            let tile = document.createElement('div');
            tile.addEventListener('click',this.selectDivEvent.bind(this));
            // tile.id = 'block-'+id;
            let currentTime = element.time;
            let sizePercentage = 40000*(currentTime - prevTime)/this.file_metadata.duration;
...
let chordHandler;
chordHandler = new ChordHandler(data);
